    ...
    with open(filename, 'r') as file:
        for line in file:
            ...
            for k in obj_metrics:
                if k in variables:
                    variables[k].append(obj_metrics[k])
                else:
                    variables[k] = [obj_metrics[k]]
...
def generate_metric_pair(metrics):
    ...
    metricvariance = np.abs(metricmean * FRAC_VARIANCE)
...
def generate_min_condition(labels, metrics):
    ret = {
        "And": [
            {
                "Or": [
        ...
        query = {
            "Select": {
                ...
                "predicate": {
                    ...
                    "condition": generate_min_condition(labels, metrics)
                }
            }
        }
